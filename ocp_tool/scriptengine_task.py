    ...
    class OCPTool(Task):
        ...
        def run(self, context):
            ...
            def oifs_oasis_grid_name(grid_type, specifier):
                return (
                    oasis_grid_names[grid_type][0]
                    + specifier
                    + oasis_grid_names[grid_type][1:]
                )

            # OpenIFS grid(s)
            oifs_grid_type = self.getarg("oifs_grid_type", context)
            try:
                oifs_grid = ocp_tool.grids.factory(oifs_grid_type)
            ...
            self.log_debug(f"OIFS grid area: {oifs_grid.cell_areas().sum():12.8e}")
            ocp_tool.oasis.write_area(
                name=oifs_oasis_grid_name(oifs_grid_type, "L"),
            ...
            ocp_tool.oasis.write_mask(
                name=oifs_oasis_grid_name(oifs_grid_type, "O"), masks=1 - oifs_lsm
            )
            ...
            if nemo_grid_file is not None:
                try:
                    nemo_grid = ocp_tool.grids.factory(
            ...
            # Runoff-mapper grid
            rnfm_grid = ocp_tool.grids.factory("F128")
            ...
            self.log_debug(f"RNFM grid area: {rnfm_grid.cell_areas().sum():12.8e}")
            ocp_tool.oasis.write_area(
                name=oasis_grid_names["rnfm-atm"], areas=rnfm_grid.cell_areas()
            )
            ...
            amipfr_grid = ocp_tool.grids.factory("regular_latlon", nlats=180, nlons=360)
            ...
            self.log_debug(f"AMIP-FR grid area: {amipfr_grid.cell_areas().sum():12.8e}")
            ocp_tool.oasis.write_area(
                name=oasis_grid_names["amipfr"], areas=amipfr_grid.cell_areas()
            )
