...
class ORCA:
    ...
    def __init__(self, domain_cfg, masks=None):
        ...
        with Dataset(domain_cfg) as nc:
            ...
            if not {
                ...
                raise RuntimeError("Missing grid variable(s) in NEMO domain config")
            try:
        ...
        if self.masks is None:
            with Dataset(domain_cfg) as nc:
                if 'top_level' not in nc.variables:
                    raise RuntimeError("Missing 'top_level' variable in NEMO domain config")
        else:
    ...
    def cell_corners(self, subgrid="t"):
        ...
        if lats.shape != lons.shape:
            raise ValueError(f"Incompatible lat/lon arrays in {self.domain_cfg}")
